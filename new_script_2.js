...
let scene, camera, renderer, effectComposer, controls, asciiPass, raycaster, model, circleModel, clock;
...
let targetCameraPosition = { x: 12, y: 6, z: -19 };
...
function init() {
    ...
    loader.load('original_golem_material.glb', function (gltf) {
        model = gltf.scene;
    ...
    loader.load('circles.glb', function (gltf) {
        circleModel = gltf.scene;
        circleModel.position.set(15, 0, 0); // Move the stones to the left
...
function createRock() {
    ...
    const randomRockName = rockNames[Math.floor(Math.random() * rockNames.length)];
...
function animate() {
    ...
    if (head) {
        ...
        if (intersects.length > 0) {
            ...
            const direction = new THREE.Vector3().subVectors(mouseWorldPosition, head.getWorldPosition(new THREE.Vector3())).normalize();
